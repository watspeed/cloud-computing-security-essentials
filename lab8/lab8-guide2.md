...
network kernel drivers; they implement network devices that are supported entirely in software. TAP (as in
...
We will run the `tun.py` program on HostU (`10.9.0.5`). 
...
Instead of using `tun` as the prefix of the interface name, use your last name as the prefix. For
example, if your last name is smith, you should use `smith` as the prefix. If your last name is long, you can
use the first five characters.

  ![tun](../images/lab7-666.png)

Make the `tun.py` program executable and run it using root privileges. 
...
container. Then print out all the interfaces on the machine by running the
following command:
...
You should be able to find an interface called `[last_name]0`.
...
commands for the configuration (replacing `tun0` with your custom interface name):
...
The following is the updated code for creating the TUN interface:
...
In this task, we will read from the TUN interface. Whatever is coming out from the TUN interface is an IP
...
After running the revised `tun.py` program on HostU and configuring the TUN interface with the assigned IP address (`192.168.53.99/24`), you can notice the following:
